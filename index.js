...
// Handle INIT — return Trade_Details screen with all options
...
// Handle Trade_Details — route to New Trade or Addendum screen
if (plain.action === 'data_exchange' && plain.screen === 'Trade_Details') {
  const category = plain.data && plain.data.trade_category;
  console.log('Trade_Details received, category:', category);
  ...
  if (category === 'addendum') {
    console.log('Fetching approved trades for dropdown...');
    let approvedTrades = [];
    try {
    ...
    const responseData = {
      ...
      data: {
        approved_trades: approvedTrades.length > 0
          ? approvedTrades
...
// Handle New_Trade_Screen submission
if (plain.action === 'data_exchange' && plain.screen === 'New_Trade_Screen') {
  console.log('New trade submitted, forwarding to Make.com...');
  try {
    ...
    console.error('Failed to forward new trade:', e.message);
  }
...
if (plain.action === 'data_exchange' && plain.screen === 'Addendum_Screen') {
  console.log('Addendum submitted, forwarding to Make.com...');
  try {
    await fetch(MAKE_WEBHOOK_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Failed to forward addendum:', e.message);
  }
...
// Fallback — return Trade_Details with options
